...
public class ForgetPassword extends BaseActivity {
    ...
    Toolbar toolbar;
    @BindView(R.id.et_input_numble)
    EditText etInputNumble;
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.bt_get_code:
                if (etInputNumble.getText().toString().length()!=11){
    ...
    private void judgeAndJumpToActivity() {
        if (etInputNumble.getText().toString().length()==11&&etInputValidationCode.getText().toString().length()==6) {
            BeanUserRegisterReq beanUserRegisterReq = new BeanUserRegisterReq();
            beanUserRegisterReq.setType(1);//1.表示修改密码
            beanUserRegisterReq.setMobileNo(etInputNumble.getText().toString());//手机号
        ...
        }else if(etInputNumble.getText().toString().length()!=11){
    ...
    private void getVerifyRequest() {
        ...
        beanUserSmsAuthReq.setMobileNo(etInputNumble.getText().toString());
