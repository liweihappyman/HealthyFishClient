...
public class Register extends AppCompatActivity implements IRegisterView {
    ...
    private void register() {
        if (getPhoneNumber().length() == 11 && getVerificationCode().length() == 6) {
            BeanUserRegisterReq beanUserRegisterReq = new BeanUserRegisterReq();
            beanUserRegisterReq.setMobileNo(getPhoneNumber());
            beanUserRegisterReq.setVerifyCode(getVerificationCode());
            ...
            Toast.makeText(Register.this, "请检查手机号码", Toast.LENGTH_LONG).show();
        } else if (getVerificationCode().length() != 6) {
    ...
    private void getVerifyRequest() {
        ...
        beanUserSmsAuthReq.setMobileNo(getPhoneNumber());
    ...
    @Override
    public void ReadingClauseAndProtocol() {
