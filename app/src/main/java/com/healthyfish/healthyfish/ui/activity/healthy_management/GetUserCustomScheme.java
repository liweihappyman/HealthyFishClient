...
public class GetUserCustomScheme extends BaseActivity {
    ...
    private void initGvDateTitle() {
        String[] strTitle = new String[]{"一", "二", "三", "四", "五", "六", "日"};
     ...
     * 计算日期
     */
    private List<String> setDate() {
        int days = 7;//可选天数
        int number;//日期对应星期的位置（0表示星期一，以此类推，6表示星期天）
        List<String> dateList = new ArrayList<>();
        SimpleDateFormat dateFormat = new SimpleDateFormat("dd");//设置想要的日期格式
        Calendar calendar = Calendar.getInstance();//获取日历实例
        Date currentDate = calendar.getTime();//获取当前的时间
        calendar.setTime(currentDate);//将获取的日期设置成当前的日期
        ...
        for (int i = 0; i < days + number; i++) {
            if (i < number) {//如果当天不是星期一，则当前星期之前的位置置空（number表示当前星期）
                dateList.add("");
            } else if (i == number) {
